...
#	print("Padding sequences....")
	...
	# compute outpur of softmax layer
...
#	print("Padding sequences....")
	...
	ALL_prob = [1]*len(y_train)

	# compute the in-domain probability, there are 9243 tweets
	softmax_output = get_softmax_output([X_test[0:9243]])[0]
	for i in range(0, 9243):
		ALL_prob.append(softmax_output[i][y_test[i]])

	#------------------------------------------------------------
	# Numf of In - Domain tweets: 9243
	# Adding 5000 tweets from Out of Domain, then compute the accuracy
			
	count = int((len(y_test) - 9243)/5000)

	for i in range (1,count+1):
		print ("---------------Model: " + str(i)+"-----------------------------")
		softmax_output = get_softmax_output([X_test[(i-1)*5000 + 9243 : 5000*i+9243]])[0]
		for j in range(0, 5000):		
			ALL_prob.append(softmax_output[j][y_test[9243+(i-1)*5000+j]])
		...
		model_name_X = options.model_dir + "cnn-adaptation-model-X-" + str(i) + "-" + optimizer + "-" + str(options.nb_filter) + "-" + \
		 str (options.tune_emb) + "-" + loss + "-" + str (options.minibatch_size) + "-" + str(options.dropout_ratio) + "-init-" + \
		 str (options.init_type) + "-" + str (options.max_features) + "-" + str (options.emb_size) + "-" + str (options.hidden_size) \
		 + ".model.cl." + str(nb_classes) + ".dom." + \	str(options.data_spec) 
		...
		model_X.fit(X_train02[0 : len(y_train) + 9243 + 5000*i], y_train_mod02[0 : len(y_train) + 9243 + 5000*i], \
				...
				callbacks=[earlystopper_X, checkpointer_X], sample_weight=np.array(ALL_prob))
	...
	# save probability	
	cPickle.dump(ALL_prob, open('save.ALL_prob', 'wb'))

...
MODEL_DIR="saved_models/cnn_models_google_emb/"
...
emb_file="../../data/word2vec_GoogleNews/GoogleNews-vectors-negative300.bin.txt.gz" 

for data in "${DATA_DIR[@]}"; do
	d_set=$(echo $data | rev | cut -d '/' -f 2 | rev);
	log="$EXP_DIR/CNN_google_emb_$d_set.log";
				...
				for filt_len in ${filt_lengths[@]}; do
					for pool_len in ${pool_lengths[@]}; do
						for vocab in ${vocab_sizes[@]}; do
							...
							python $CNN_SCR --data-dir=$data --model-dir=$MODEL_DIR -i $init_type -f $emb_file\

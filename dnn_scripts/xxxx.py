...
#    	,featFile_dev   = "../data/good_vs_bad/CQA-QL-devel.xml.multi.csv.feat"
	...
	nb_words=options.max_features, init_type=options.init_type, embfile=options.emb_file, dev_train_merge=0, map_labels_to_five_class=0)
...
#	print("Padding sequences....")
	...
	model_name = options.model_dir + "cnn" + "-" + optimizer + "-" + str(options.nb_filter) + "-" + str (options.tune_emb) +\
...
#		score, acc = model.evaluate(X_test, y_test_mod, batch_size=options.minibatch_size, show_accuracy=True)
#		print('Test accuracy:', acc)
	

	print ("**********get output of softmax layer, compute sample_weight************************")
	get_softmax_output = K.function([model.layers[0].get_input(train=False)],[model.layers[10].get_output(train=False)])
	#print ("------------------------------------------")
	#print (softmax_output)
	#print (len(softmax_output))
	#print ("------------------------------------------")



	######################################################################
	adaptaion_dir = "/alt/work/ndat/AIDR/dataset/adapt_expt/in-out"
	 
	print('Loading in + out domain data...')
	(X_train02, y_train02), (X_test02, y_test02), (X_dev02, y_dev02), max_features, E, label_id = \
	aidr.load_and_numberize_data(adaptaion_dir,\
	nb_words=options.max_features, init_type=options.init_type, embfile=options.emb_file, dev_train_merge=0, map_labels_to_five_class=0)

#	print("Padding sequences....")
	X_train02 = sequence.pad_sequences(X_train02, maxlen=options.maxlen)
	X_test02  = sequence.pad_sequences(X_test02,  maxlen=options.maxlen)
	X_dev02   = sequence.pad_sequences(X_dev02,   maxlen=options.maxlen)


	#build model...
	nb_classes = np.max(y_train02) + 1

	print('............................')
	print(len(X_train02), 'train tweets')
	print(len(X_test02),  'test  tweets')
	print(len(X_dev02),   'dev   tweets')
	print(max_features - 3, 'vocabulary size')
	print(nb_classes, 'different classes')
	print('............................')
	...
	if nb_classes == 2: # binary
		...
		# convert class vectors to binary class matrices [ 1 of K encoding]
		y_train_mod02 = np_utils.to_categorical(y_train02, nb_classes)
		y_test_mod02  = np_utils.to_categorical(y_test02,  nb_classes)
		y_dev_mod02   = np_utils.to_categorical(y_dev02,   nb_classes)



			
	count = int((len(y_test) - 9243)/5000)

	for i in range (1,count+1):
		print ("---------------Model: " + str(i)+"-----------------------------")
		
		#compute deferencet mda
		model_X = build_cnn(options.maxlen, max_features, emb_matrix=E, emb_size=options.emb_size, nb_filter=options.nb_filter,
						filter_length=options.filter_length, pool_length=options.pool_length, nb_classes = nb_classes,
						hidden_size=options.hidden_size, dropout_ratio=options.dropout_ratio, tune_emb=options.tune_emb)

		model_X.compile(optimizer=optimizer, loss=loss,  class_mode=class_mode)

		model_name_X = options.model_dir + "cnn-adaptation-model-X-" + str(i) + "-" + optimizer + "-" + str(options.nb_filter) + "-" + str (options.tune_emb) +\
		"-" + loss + "-" + str (options.minibatch_size) + "-" + str(options.dropout_ratio) + "-init-" + str (options.init_type) + "-" +\
		str (options.max_features) + "-" + str (options.emb_size) + "-" + str (options.hidden_size) + ".model.cl." + str(nb_classes) + ".dom." + \
		str(options.data_spec) 

		earlystopper_X = EarlyStopping(monitor='val_loss', patience=3, verbose=1)
		checkpointer_X = ModelCheckpoint(filepath=model_name_X, monitor='val_loss', verbose=1, save_best_only=True)


		print ('Training and validating ....')
		model_X.fit(X_train02[0 : len(y_train) + 9243 + 5000*i], y_train_mod02[0 : len(y_train) + 9243 + 5000*i], \
				batch_size=options.minibatch_size, nb_epoch=options.epochs,
				shuffle=True, validation_data=(X_dev02, y_dev_mod02), show_accuracy=True, verbose=2, \
				callbacks=[earlystopper_X, checkpointer_X])

		print("Test model ...")
		print ("Loading ...", model_name_X)
		model_X.load_weights(model_name_X)

	
		y_pred02 = model_X.predict_classes(X_test02)
		y_test02 = np.array(y_test02)
	
		acc2 = metrics.accuracy_score(y_test02, y_pred02)
		print("Raw Accuracy Model " + str(i) + " is: " + str(acc2))
	...
	print ("----------------Finishing compute probablility---------------")
	'''
	adaptaion_dir = "/alt/work/ndat/AIDR/dataset/adapt_expt/in-out"
...
#	print("Padding sequences....")
	...
	print ("****************************Adaptaion Model**********************************")

	model02 = build_cnn(options.maxlen, max_features, emb_matrix=E, emb_size=options.emb_size, nb_filter=options.nb_filter,
						filter_length=options.filter_length, pool_length=options.pool_length, nb_classes = nb_classes,
						hidden_size=options.hidden_size, dropout_ratio=options.dropout_ratio, tune_emb=options.tune_emb)

	model02.compile(optimizer=optimizer, loss=loss,  class_mode=class_mode)

	model_name02 = options.model_dir + "cnn-adaptation" + "-" + optimizer + "-" + str(options.nb_filter) + "-" + str (options.tune_emb) +\
	"-" + loss + "-" + str (options.minibatch_size) + "-" + str(options.dropout_ratio) + "-init-" + str (options.init_type) + "-" +\
	str (options.max_features) + "-" + str (options.emb_size) + "-" + str (options.hidden_size) + ".model.cl." + str(nb_classes) + ".dom." + \
	str(options.data_spec) 

	earlystopper02 = EarlyStopping(monitor='val_loss', patience=3, verbose=1)
	checkpointer02 = ModelCheckpoint(filepath=model_name02, monitor='val_loss', verbose=1, save_best_only=True)


	print ('Training and validating ....')
	model02.fit(X_train02, y_train_mod02, batch_size=options.minibatch_size, nb_epoch=options.epochs,
				shuffle=True, validation_data=(X_dev02, y_dev_mod02), show_accuracy=True, verbose=2, \
				callbacks=[earlystopper02, checkpointer02], sample_weight=prob_)

	print("Test model ...")
	print ("Loading ...", model_name02)
	model02.load_weights(model_name02)

	
	y_pred02 = model02.predict_classes(X_test02)
	y_test02 = np.array(y_test02)
	...
	print (metrics.classification_report(y_test02, y_pred02, labels=class_ids, target_names=p_labels, digits=3) )
	#print (metrics.classification_report(y_test, y_pred, labels=class_ids, digits=3) )


	print ("Confusion Matrix:\n", metrics.confusion_matrix(y_test02, y_pred02, labels=class_ids))

	mic_p, mic_r, mic_f, sup = metrics.precision_recall_fscore_support(y_test02, y_pred02, average='micro')
	mac_p, mac_r, mac_f, sup = metrics.precision_recall_fscore_support(y_test02, y_pred02, average='macro')
...
###############################################################################################################
	...
	model03 = build_cnn(options.maxlen, max_features, emb_matrix=E, emb_size=options.emb_size, nb_filter=options.nb_filter,
						filter_length=options.filter_length, pool_length=options.pool_length, nb_classes = nb_classes,
						hidden_size=options.hidden_size, dropout_ratio=options.dropout_ratio, tune_emb=options.tune_emb)

	model03.compile(optimizer=optimizer, loss=loss,  class_mode=class_mode)

	model_name03 = options.model_dir + "cnn-normal" + "-" + optimizer + "-" + str(options.nb_filter) + "-" + str (options.tune_emb) +\
	"-" + loss + "-" + str (options.minibatch_size) + "-" + str(options.dropout_ratio) + "-init-" + str (options.init_type) + "-" +\
	str (options.max_features) + "-" + str (options.emb_size) + "-" + str (options.hidden_size) + ".model.cl." + str(nb_classes) + ".dom." + \
	str(options.data_spec) 

	earlystopper03 = EarlyStopping(monitor='val_loss', patience=3, verbose=1)
	checkpointer03 = ModelCheckpoint(filepath=model_name03, monitor='val_loss', verbose=1, save_best_only=True)


	print ('Training and validating ....')
	model03.fit(X_train02, y_train_mod02, batch_size=options.minibatch_size, nb_epoch=options.epochs,
				shuffle=True, validation_data=(X_dev02, y_dev_mod02), show_accuracy=True, verbose=2, \
				callbacks=[earlystopper03, checkpointer03])

	print("Test model ...")
	print ("Loading ...", model_name03)
	model03.load_weights(model_name03)

	
	y_pred02 = model03.predict_classes(X_test02)
	y_test02 = np.array(y_test02)

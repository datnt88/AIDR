...
#	print("Padding sequences....")
	...
	print (metrics.classification_report(y_test, y_pred, labels=class_ids, target_names=p_labels, digits=3) )
	...
	model_name02 = options.model_dir + "cnn-adpataion" + "-" + optimizer + "-" + str(options.nb_filter) + "-" + str (options.tune_emb) +\
	"-" + loss + "-" + str (options.minibatch_size) + "-" + str(options.dropout_ratio) + "-init-" + str (options.init_type) + "-" +\
	str (options.max_features) + "-" + str (options.emb_size) + "-" + str (options.hidden_size) + ".model.cl." + str(nb_classes) + ".dom." + str(options.data_spec) 
	...
	all_prob = cPickle.load(open('save.prob', 'rb'))
	print ("----------------------loading done-------------------------")


	# multi-class
	print ('Training and validating ....')
	model02.fit(X_train, y_train_mod, batch_size=options.minibatch_size, nb_epoch=options.epochs,
				validation_data=(X_dev, y_dev_mod), show_accuracy=True, verbose=2, \
				shuffle=True, callbacks=[earlystopper02, checkpointer02], sample_weight=all_prob)

	print("Test model ...")
	print ("Loading ...", model_name02)
	model02.load_weights(model_name02)

	y_pred = model02.predict_classes(X_test)
	y_test = np.array(y_test)

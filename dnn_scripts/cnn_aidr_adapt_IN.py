...
#	print("Padding sequences....")
	...
	# because nb_classes > 2: # multi-class
	...
	softmax_output = get_softmax_output([X_test])[0]
	
	
	# assign probability = 1 for Nepal training tweets
	prob_ = [1]* len(y_train)

	# compute probability for each in-domain tweets
	for i in range(0, len(y_test)):
		prob_.append( 1.0 -softmax_output[i][y_test[i]])			
	print (prob_)

	prob_ = np.array(prob_)
	cPickle.dump(prob_, open('save.prob', 'wb'))
	cPickle.dump(softmax_output, open('save.softmax', 'wb'))	
	cPickle.dump(y_test, open('save.y_test', 'wb'))	
	print ("----------------Finishing compute probablility---------------")



	# -------------------------------applying domain adaption--------------------
	# in- domain data 
	adaptaion_dir = "/alt/work/ndat/AIDR/dataset/adapt_expt/in"

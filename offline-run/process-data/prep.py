...
#=============
prccd_folder = "./"
#prccd_folder = "~/Dropbox (QCRI)/AIDR-DA-ALT-SC/sigir2016/data/out-domain/gold_silver/" #no backslashes in front of special characters like spaces
...
def main(csvfile):
	...
	name=(os.path.splitext(csvfile)[0])
        ##open
        with open(prccd_folder+"%s_prccd.csv" % name, 'wb') as f:
                writer = csv.writer(f)
#		writer.writerow(["item_id","item","label"])
		writer.writerow(header)
		rows = zip(columns['item_id'],prccd_item_list,columns['label'],columns['crisis'])
		for row in rows:
			writer.writerow(row)

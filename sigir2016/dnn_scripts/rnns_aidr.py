...
def build_lstm_rnns(model_type, maxlen, max_features, emb_size=128, emb_matrix=None,
					recur_type='lstm', nb_classes = 2, recur_size=128, dropout_ratio=0.5, tune_emb=True):

	''' run lstm rnns '''

	print('Building model:', model_type, 'lstm')
	...
	if model_type.lower() == 'bidirectional':
		model = Graph()
		model.add_input(name='input', input_shape=(maxlen,), dtype=int) # add word id (input) layer; in this case the size is fixed
		model.add_node(emb_layer,    name='embedding', input='input') 		# add the emb node
		model.add_node(recur_layer,  name='forward',   input='embedding')  # fwd lstm layer
		...
		model.add_node(recur_layer2, name='backward', input='embedding') # bwd lstm layer
...
#	parser.add_option("-v", "--validation-file",   dest="featFile_dev")
	...
	parser.set_defaults(
#    	data_dir        = "/Users/joty 1/Dropbox (QCRI)/QCRI/ALT-MT/AIDR_DA/WV_NN/data/"
...
#	print("Padding sequences....")
	...
	model = build_lstm_rnns(options.model_type, options.maxlen, max_features, emb_matrix=E,
							emb_size=options.emb_size, recur_type=options.recur_type, nb_classes = nb_classes,
							recur_size=options.hidden_size, dropout_ratio=options.dropout_ratio, tune_emb=options.tune_emb)

	model_name = options.model_dir + options.recur_type + "-" + options.model_type + "-" + optimizer + "-" + str (options.tune_emb) +\
